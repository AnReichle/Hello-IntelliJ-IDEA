public class HelloIntelliJ {

    public static void main(String[] args) {
        System.out.println("dfhjfdfbfh hfhf fhfj jfj jfjffjfj jfjfjfjfjfjfjfjf");

        System.out.println("Hello, IntelliJ!");
        System.out.println("Goodbye, IntelliJ!");
        System.out.println("I will not be back");
        System.out.println("So don't wait for me");
        System.out.println("I forgot my violine!");
        System.out.println("This happens to me all the time ...");
        System.out.println("It's not even funny anymore ... ");
        System.out.println("The worst thing is: it's not even a violine. It's a bass!");
        System.out.println("I always mix those two up ...");
        System.out.println("And I am a professional musician!!!");
        System.out.println("When I was young I wanted to be a bricklayer ... ");
        System.out.println("But my father didn't think I was very talentented at bricklaying.");

        System.out.println("Let me eat this cookie.");

        System.out.println("dsghdfi");
        System.out.println("dfghkdfgdfgjkjk     dfjkgfjkgfhgkf");

        System.out.println("I think I'll have another one.");

        System.out.println("rhjsbgrfdjgbfdjg");
        System.out.println("fgjehrdbfgjbvdrt");
        System.out.println("rfbrgbrgbgfjbfj");
    }

}
